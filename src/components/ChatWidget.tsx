...
export function ChatWidget() {
  ...
  // Load agent — exact JotForm embed, no constraints
  useEffect(() => {
    if (overlay !== "chat" || agentLoaded.current || !agentRef.current) return;

    const el = agentRef.current;

    // Create iframe exactly matching JotForm's embed code
  ...
  return (
    <>
      ...
      {/* ── VIDEO OVERLAY ── */}
      {overlay === "video" && (
        <div className="h2h-overlay" onClick={() => setOverlay(null)} role="dialog" aria-modal="true">
      ...
      <style>{`
        ...
        .h2h-chat-stack { display: flex; flex-direction: column; align-items: center; gap: 10px; }
        ...
        .h2h-chat-us:hover { transform: scale(1.05); box-shadow: 0 6px 24px rgba(164,108,252,0.5); }
        ...
        .h2h-chat-fab__dot { position: absolute; right: 4px; bottom: 4px; width: 14px; height: 14px; border-radius: 50%; background: #22c55e; border: 2px solid #fff; animation: dot 2s ease-out infinite; }
        ...
        .h2h-overlay {
          ...
          padding: clamp(12px,3vw,32px); animation: fadeIn 0.25s ease-out;
          overflow-y: auto;
        }
        ...
        .h2h-overlay__label {
          ...
          font-family: var(--font-stack-heading, system-ui, sans-serif);
          font-size: clamp(0.85rem,1.3vw,1rem); font-weight: 700;
          letter-spacing: 0.14em; text-transform: uppercase;
          padding-bottom: 6px; border-bottom: 3px solid var(--color-secondary, #a46cfc);
        }
        ...
        .h2h-overlay__modal {
          ...
          overflow: hidden; animation: popIn 0.35s cubic-bezier(0.22,1,0.36,1);
        }
        .h2h-overlay__video-stage { background: #000; aspect-ratio: 16/9; width: 100%; display: flex; align-items: center; justify-content: center; }
        .h2h-overlay__video { width: 100%; height: 100%; object-fit: contain; display: block; }

        /* ── Chat modal — no height constraints ── */
        .h2h-overlay__chat {
